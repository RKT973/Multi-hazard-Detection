...
DISPLAY_HAZARD = {
    ...
    'CONTROLLED_FIRE': 'CONTROLLED_FIRE',
    'GAS_LEAK': 'GAS_LEAK',
    ...
    for label, prediction in results.items():
        ...
        if label == 'Fire CV model':
            if class_name == 'uncontrolled_fire':
                ...
                scores['CONTROLLED_FIRE'] = max(scores['CONTROLLED_FIRE'], confidence)
...
def _cv_hazard_from_scores(cv_scores):
    if cv_scores.get('DANGER', 0.0) >= 0.92:
        return 'DANGER'
    if cv_scores.get('CONTROLLED_FIRE', 0.0) >= 0.50:
        return 'CONTROLLED_FIRE'
...
def _hazard_is_active(hazard, score, sensor_scores, cv_scores):
    ...
    if hazard == 'CONTROLLED_FIRE':
        return cv_support >= 0.50
    if hazard == 'DANGER':
...
def update_fused_hazard():
    ...
    hazards = {'DANGER', 'CONTROLLED_FIRE', 'SMOKE', 'GAS_LEAK', 'WATER_LEAK', 'OVERHEATING'}
    fusion_scores = {}
